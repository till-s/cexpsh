...
pmelf_sym_name(Pmelf_Symtab symt, Elf_Sym *sym)
{
	...
	if ( ELFCLASS64 == symt->clss ) {
		if ( sym->t64.st_name > symt->strtablen )
			return 0;
		return &symt->strtab[sym->t64.st_name];
	}
#endif
	if ( sym->t32.st_name > symt->strtablen )
		return 0;
	return &symt->strtab[sym->t32.st_name];
}
