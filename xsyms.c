...
int        elf64 = 0;
	...
	if ( ! (elfi = pmelf_newstrm(ifilen, 0)) ) {
		goto cleanup;
	}
...
static int
symfilter(bfd *abfd, asymbol *s, int flags)
{
asection *sect = bfd_get_section(s);
	...
	if ( BSF_LOCAL & s->flags ) {
		...
		if ( (FLTFLAG_SECTSYMS & flags) && (BSF_SECTION_SYM & s->flags) && (SEC_ALLOC & bfd_get_section_flags(abfd,sect)) ) {
			return 1;
		} else {
			return 0;
		}
	...
	if ( bfd_is_und_section(sect) ) {
		 ...
		 * linux platform.)
		 */
		return (FLTFLAG_UNDFSYMS & flags) || (bfd_asymbol_value(s) && (BSF_FUNCTION & s->flags));
	}
	return 1;
}
...
int                         fltflags=0;
	...
	/* scan options */
	while ( (i=getopt(argc, argv, "ahpszCU")) > 0 ) {
		switch (i) {
			...
			case 's': fltflags |= FLTFLAG_SECTSYMS; break;
			case 'U': fltflags |= FLTFLAG_UNDFSYMS; break;
		}
	}
